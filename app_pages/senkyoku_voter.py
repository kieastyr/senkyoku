...
try:
    ...
    df_combinations = df_combinations.loc[df_combinations["削除フラグ"] == 0]
...
if selected_id is not None:
    ...
    with col1:
        ...
        def format_bool(val):
            return (
                "有" if val and str(val).lower() in ["true", "1", "有", "yes"] else "無"
            )

        st.write(f"**コンチェルト:** {format_bool(detail_row.get('コンチェルト有'))}")
        st.write(f"**楽譜調達:** {format_bool(detail_row.get('楽譜調達有'))}")
        st.write(
            f"**演奏会費増額分:** {int(pd.to_numeric(detail_row.get('演奏会費増額分', 0))):,} 円"
        )
    ...
    if submit_button:
        if len({pos1, pos2, pos3, neg1}) < 4:
            ...
            try:
                ...
                logger.info(
                    f"{client_info} Vote received: 👍({pos1}, {pos2}, {pos3}), 👎({neg1})"
                )
